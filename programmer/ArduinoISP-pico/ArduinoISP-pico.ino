...
void setup() {
  ...
  //Reasonable LED colours are:
  //  ERR:   Red
  //  PMODE: Yellow
  //  HB:    Green
  // which are nicely 'traffic light'. So, let's flash them in traffic light order (r->y->g), as
  // we then enter HB mode, which leaves the green glowing.

  // We've now made the pulse 1/10th second each, so pulse each for 1s, as I like it to be
  // darned obvious we are booting and working.
...
//Make a pulse 'cycle' 1/10th of a second, so 50ms on 50ms off
#define PTIME 50
void pulse(int pin, int times) {
  do {
